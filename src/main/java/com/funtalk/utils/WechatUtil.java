...
import com.funtalk.pojo.WxTransfersDto;
...
public class WechatUtil
{
    ...
    private static final String TRANS_URL =
     ...
     */
    public static Result doTransfers(String appkey, String certPath, WxTransfersDto  model)
    {
        ...
        {
            // 1.计算参数签名
            String paramStr = WechatUtil.createLinkString(model);
            ...
            // 3.加载证书请求接口
            String result = HttpReqHandler.httpsRequest(TRANS_URL, reqXmlStr.toString(),
                model, CERT_PATH);
            LOG.info(("response xml = " + result));
            if(result.contains("CDATA[FAIL]")){
                return new Result(300, "调用微信接口失败, 具体信息请查看访问日志");
            }
        }
        catch (Exception e)
        {
            e.printStackTrace();
            return new Result(300, e.getMessage());
        }
        return new Result(Result.OK,"正常");
    }
    
    private static String createLinkString(WxTransfersDto  model)
    {
        ...
        model.setNonce_str(WechatUtil.getNonce_str());
